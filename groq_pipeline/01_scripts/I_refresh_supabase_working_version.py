...
"""
Supabase Groq Data Refresh Script
=================================
...
def setup_logging():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write(f"Supabase Working Version Refresh Log\n")
...
def get_existing_groq_count(client: Client) -> Optional[int]:
    """
    Get current count of Groq records in Supabase.
    ...
    try:
        ...
        logger.info(f"📊 Current Groq records in Supabase: {count}")
        return count
        
    except Exception as e:
        logger.error(f"❌ Failed to count existing Groq records: {str(e)}")
        return None


def backup_existing_groq_data(client: Client) -> Optional[List[Dict[str, Any]]]:
    """
    Backup existing Groq records before deletion for rollback capability.
    ...
    """
    logger.info(f"💾 Backing up existing Groq records from {TABLE_NAME}...")
    ...
    try:
        ...
        logger.info(f"✅ Backed up {len(backup_data)} existing Groq records")
        return backup_data
        
    except Exception as e:
        logger.error(f"❌ Failed to backup existing Groq records: {str(e)}")
        return None


def delete_existing_groq_data(client: Client) -> bool:
    """
    Delete existing Groq records from Supabase working_version table.
    ...
    """
    logger.info(f"🗑️ Deleting existing Groq records from {TABLE_NAME}...")
    
    try:
        # Get count before deletion for verification
        initial_count = get_existing_groq_count(client)
        if initial_count is None:
            return False
            
        if initial_count == 0:
            logger.info("✅ No existing Groq records to delete")
            return True
        
        # Delete all Groq records
        response = client.table(TABLE_NAME).delete().eq("inference_provider", INFERENCE_PROVIDER).execute()
        
        # Verify deletion
        remaining_count = get_existing_groq_count(client)
        if remaining_count == 0:
            logger.info(f"✅ Successfully deleted {initial_count} Groq records")
            return True
        else:
        ...
        logger.error(f"❌ Failed to delete Groq records: {str(e)}")
        return False


def restore_backup_data(client: Client, backup_data: List[Dict[str, Any]]) -> bool:
    """
    Restore backed up Groq records to Supabase (rollback operation).
    ...
    """
    logger.info(f"🔄 Rolling back: Restoring {len(backup_data)} Groq records...")
    ...
    try:
        ...
        # Verify restoration
        final_count = get_existing_groq_count(client)
        if final_count >= len(backup_data) * 0.95:  # Allow 5% tolerance due to potential data conflicts
            logger.info(f"✅ Rollback successful: {final_count} Groq records restored")
...
def load_normalized_csv() -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        # Expected JSON fields (matches Supabase schema exactly)
            ...
            # Load all records (no filtering needed - CSV contains only Groq)
        ...
        logger.info(f"✅ Loaded {len(models)} Groq models from CSV")
        return models
...
def main():
    ...
    """
    logger.info("=" * 60)
    logger.info("SUPABASE GROQ DATA REFRESH STARTED")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("🎉 SUPABASE GROQ DATA REFRESH COMPLETED SUCCESSFULLY")
        logger.info("=" * 60)
        logger.info(f"📊 Summary:")
        logger.info(f"   • Initial Groq records: {initial_count}")
