...
LOG_FILE = SCRIPT_DIR / "deploy_to_ai_models_main_log.txt"
...
def get_google_count(client: Client, table_name: str) -> Optional[int]:
    """
    Get current count of Google records in specified table.
    ...
    try:
        ...
        logger.info(f"📊 Current Google records in {table_name}: {count}")
        return count
        
    except Exception as e:
        logger.error(f"❌ Failed to count Google records in {table_name}: {str(e)}")
        return None


def backup_production_google_data(client: Client) -> Optional[List[Dict[str, Any]]]:
    """
    Backup existing Google records from ai_models_main for rollback capability.
    ...
    """
    logger.info(f"💾 Backing up existing Google records from {PRODUCTION_TABLE}...")
    ...
    try:
        ...
        logger.error(f"❌ Failed to backup existing Google records: {str(e)}")
        return None


def delete_production_google_data(client: Client) -> bool:
    """
    Delete existing Google records from ai_models_main table.
    ...
    try:
        ...
        initial_count = get_google_count(client, PRODUCTION_TABLE)
        if initial_count is None:
            return False
            
        if initial_count == 0:
            logger.info("✅ No existing Google records to delete from production")
            return True
        
        # Delete all Google records
        response = client.table(PRODUCTION_TABLE).delete().eq("inference_provider", INFERENCE_PROVIDER).execute()
        
        # Verify deletion
        remaining_count = get_google_count(client, PRODUCTION_TABLE)
        if remaining_count == 0:
            logger.info(f"✅ Successfully deleted {initial_count} Google records from production")
            return True
        else:
        ...
        logger.error(f"❌ Failed to delete Google records from production: {str(e)}")
        return False


def load_staging_google_data(client: Client) -> Optional[List[Dict[str, Any]]]:
    """
    Load Google data from working_version table for deployment.
    ...
    try:
        ...
        if not staging_data:
            logger.error(f"❌ No Google data found in {STAGING_TABLE}")
            return None
        ...
        logger.error(f"❌ Failed to load Google data from staging: {str(e)}")
        return None
...
def restore_production_backup(client: Client, backup_data: List[Dict[str, Any]]) -> bool:
    """
    Restore backed up Google records to ai_models_main (rollback operation).
    ...
    try:
        ...
        # Verify restoration
        final_count = get_google_count(client, PRODUCTION_TABLE)
...
def main():
    ...
    try:
        ...
        staging_count = get_google_count(client, STAGING_TABLE)
        production_count = get_google_count(client, PRODUCTION_TABLE)
        ...
        staging_data = load_staging_google_data(client)
        if not staging_data:
        ...
        backup_data = backup_production_google_data(client)
        ...
        if not delete_production_google_data(client):
