...
def setup_logging():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write(f"Supabase Working Version Refresh Report\n")
...
def get_existing_google_count(client: Client) -> Optional[int]:
    """
    Get current count of Google records in Supabase.
    ...
    try:
        ...
        logger.info(f"📊 Current Google records in Supabase: {count}")
        return count
        
    except Exception as e:
        logger.error(f"❌ Failed to count existing Google records: {str(e)}")
        return None


def backup_existing_google_data(client: Client) -> Optional[List[Dict[str, Any]]]:
    """
    Backup existing Google records before deletion for rollback capability.
    
    Args:
        client: Supabase client instance
        
    Returns:
        List[Dict]: Backup of existing Google records
        None: If backup fails
    """
    logger.info(f"💾 Backing up existing Google records from {TABLE_NAME}...")
    ...
    try:
        ...
        logger.info(f"✅ Backed up {len(backup_data)} existing Google records")
        return backup_data
...
def delete_existing_google_data(client: Client) -> bool:
    """
    Delete existing Google records from Supabase working_version table.
    ...
    """
    logger.info(f"🗑️ Deleting existing Google records from {TABLE_NAME}...")
    
    try:
        # Get count before deletion for verification
        initial_count = get_existing_google_count(client)
        if initial_count is None:
            return False
            
        if initial_count == 0:
            logger.info("✅ No existing Google records to delete")
            return True
        ...
        # Verify deletion
        remaining_count = get_existing_google_count(client)
        if remaining_count == 0:
            logger.info(f"✅ Successfully deleted {initial_count} Google records")
            return True
        else:
        ...
        logger.error(f"❌ Failed to delete Google records: {str(e)}")
        return False


def restore_backup_data(client: Client, backup_data: List[Dict[str, Any]]) -> bool:
    """
    Restore backed up Google records to Supabase (rollback operation).
    ...
    """
    logger.info(f"🔄 Rolling back: Restoring {len(backup_data)} Google records...")
    ...
    try:
        ...
        # Verify restoration
        final_count = get_existing_google_count(client)
        if final_count >= len(backup_data) * 0.95:  # Allow 5% tolerance due to potential data conflicts
            logger.info(f"✅ Rollback successful: {final_count} Google records restored")
...
def main():
    ...
    """
    logger.info("=" * 60)
    logger.info("SUPABASE GOOGLE DATA REFRESH STARTED")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("🎉 SUPABASE GOOGLE DATA REFRESH COMPLETED SUCCESSFULLY")
        logger.info("=" * 60)
        logger.info(f"📊 Summary:")
        logger.info(f"   • Initial Google records: {initial_count}")
